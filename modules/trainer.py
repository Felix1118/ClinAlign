...
class BaseTrainer(object):
    def __init__(self, model, criterion_cls, base_probs, metric_ftns, args, device, is_main_process):
        ...
        print("number of trainable parameters: {}M".format(num_parameters//1000000))
        optim_params = [
            {
                "params": p_wd,
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.is_main_process:
                ...
                if log[self.mnt_metric] >= self.mnt_best:
                    ...
                    best_path = os.path.join(self.checkpoint_dir, 'model_best_epoch.pth')
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch_blip(self, epoch):
        train_loss = 0
        self.model.train()
        for batch_idx, (images, final_prompts,captions, cls_labels, clip_memory) in enumerate(self.train_dataloader):
            ...
            loss_lm, loss_cls = self.model(images,final_prompts, captions, cls_labels, clip_memory, self.criterion_cls, self.base_probs)
    ...
    def eval_blip(self, log):
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            pbar = tqdm(self.val_dataloader, desc="Validating", ncols=100)
            for batch_idx, (images, captions, cls_labels, clip_memory) in enumerate(pbar): 
                images = images.to(self.device) 
                cls_labels = cls_labels.to(self.device)
                clip_memory = clip_memory.to(self.device)
